...
app.post('/produtos', async(req,res)=>{  
    await produto.create(    
       req.body
        ...
        return res.status(400).json({
            error: true,
            message: "Foi impossível criar o produto."
        })
    });
});
...
app.put('/atualizaproduto', async(req,res)=>{
    await produto.update(req.body,{
        ...
        return res.status(400).json({
            error: true,
            message: "Houve erro na alteção do produto."
        });
    });    
});

//exclusão de produtos
app.get('/excluir-produto/:id', async(req,res)=>{
    await produto.destroy({
        ...
        return res.status(400).json({
            error: true,
            message: "Impossível excluir o produto."
        });
    });
});
...
//atualizando compra
app.put('/cliente/:id/compra', async(req,res)=>{
    const comp = {
        ClienteId: req.params.id,
        data: req.body.data
    };
...
//exclusão de compra
app.get('/excluir-compra/:id', async(req,res)=>{
    await compra.destroy({
        ...
        return res.status(400).json({
            error: true,
            message: "Impossível excluir a compra."
        });
    });
});
...
app.get('/lista-item-compras', async(req,res)=>{ 
...
app.get('/excluir-itens-compra/:id', async(req,res)=>{
    const item={
        quantidade: req.body.quantidade,
        valor: req.body.valor
    };
    await itemcompra.destroy(item,{
        where: Sequelize.and({ProdutoId: req.body.ProdutoId},
        {CompraId: req.params.id})
    }).then(function(itens){
        return res.json({
        ...
        return res.status(400).json({
            error: true,
            message: "Impossível excluir o item."
        });
    });
});
